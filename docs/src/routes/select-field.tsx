...
const examples = [
	...
	},
	{
		...
		description: {
			text: 'Demonstration of default values working with different data types',
			highlight: ['defaultValue', 'options'],
		},
		preview: () => {
			const numberOptions = [
				{ label: 'One', value: 1 },
			...
			return (
				<div class='space-y-4 w-full max-w-sm'>
					...
					<SelectField<string[]>
						...
						options={arrayOptions}
						defaultValue={['read', 'write', 'deploy']}
						onSelected={(value) => logValue('Array selection', value)}
						help='Defaults to "Developer Access"'
					/>

					<SelectField
						name='object'
						label='Object with Default'
						options={objectOptions}
						defaultValue={{
							id: 2,
							role: 'editor',
						...
						onSelected={(value) => logValue('Object selection', value)}
						help='Defaults to "Editor User"'
					/>

					<div class='p-4 bg-gray-100 dark:bg-gray-800 rounded-lg'>
						<ol class='list-decimal list-inside text-sm text-gray-700 dark:text-gray-300 space-y-1'>
							<li>Each select starts with a pre-selected value</li>
							<li>Values maintain their type when changed</li>
							<li>Check console logs to verify type preservation</li>
						</ol>
					</div>
				</div>
			);
